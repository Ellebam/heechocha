 ...
 */
export function buildChatgptUrl(model) {
  // If model is "default" or explicitly empty, just open main page
...
export function parseGeminiUrl(url) {
  ...
  // Or: https://gemini.google.com/gem/27f573a8f861 (no account index)
  const match = url.match(/gemini\.google\.com\/(?:u\/(\d+)\/)?gem\/([a-zA-Z0-9]+)/);
  if (match) {
    return {
      accountIndex: match[1] ? parseInt(match[1], 10) : null,
      gemId: match[2]
    };
  }
  return null;
...
export const CHATGPT_MODELS = [
  ...
  { id: 'gpt-4o-mini', name: 'GPT-4o Mini', description: 'Fast and lighter' },
  { id: 'o1-pro', name: 'o1 Pro', description: 'Reasoning model' }
];